...
# fetch-template.sh
#
# Pulls the latest hfwgtemplate.tex, default.csl, and font files from the
# hfwg-tex-template GitHub repository (main branch) into a local managed
# directory.  Run whenever you want to update to the current version.
#
# Usage:
#   bash fetch-template.sh                        # update ~/Templates/hfwg-tex/
#   HFWG_TEMPLATE_DIR=/custom/path bash fetch-template.sh
#   HFWG_BRANCH=v1.0.0 bash fetch-template.sh      # pin to a specific tag
#
...
REPO="https://raw.githubusercontent.com/jncohen/HFWG-tex-template"
BRANCH="${HFWG_BRANCH:-main}"
BASE="$REPO/$BRANCH"
DEST="${HFWG_TEMPLATE_DIR:-$HOME/Templates/hfwg-tex}"

echo "Fetching hfwg-tex-template ($BRANCH) to: $DEST"
mkdir -p "$DEST/fonts"
...
# --- Core files ---
fetch "$BASE/inst/templates/hfwgtemplate.tex"  "$DEST/hfwgtemplate.tex"
...
echo "  template: $DEST/hfwgtemplate.tex"
