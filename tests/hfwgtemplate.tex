...
\usepackage{xcolor}
$if(accent)$
  \definecolor{hfwgaccent}{HTML}{$accent$}
  \definecolor{hfwgrule}{HTML}{$accent$}
$else$
  \ifhfwgmethods
...
$if(numbersections)$
  ...
  \else\ifhfwghfwg
    \titleformat{\section}{\large\bfseries\color{hfwgaccent}}{\thesection}{1em}{}[\hfwgwgrule\vspace{0.4em}]
    \titleformat{\subsection}{\normalsize\bfseries\color{hfwgaccent}}{\thesubsection}{1em}{}
    \titleformat{\subsubsection}{\normalsize\itshape}{\thesubsubsection}{1em}{}
    \titlespacing{\section}{0pt}{2ex plus 0.5ex}{-0.5ex}
    \titlespacing{\subsection}{0pt}{1.5ex plus 0.25ex}{-0.5ex}
...
\else\ifhfwghfwg
  \renewcommand{\headrule}{%
    {\color{hfwgrule}\hrule width\textwidth height\headrulewidth}%
    \vskip-\headrulewidth%
  }
\fi\fi
...
{\small%
$if(author.dept)$$author.dept$\par$endif$%
$if(author.inst)$$author.inst$\par$endif$%
$if(author.addr)$$author.addr$\par$endif$%
$if(author.email)$$author.email$\par$endif$%
$if(author.web)$$author.web$\par$endif$%
$if(author.orcid)$ORCID: $author.orcid$\par$endif$%
}}%
$else$%
...
\fcolorbox{hfwgaccent}{white}{%
  \begin{minipage}[c][0.72in][c]{0.86\linewidth}
  \centering\rule{0pt}{0.68in}
  \end{minipage}%
}
\end{minipage}\hfill
\begin{minipage}[t]{0.78\textwidth}
\vspace{0pt}
{\small\sffamily\bfseries\color{hfwgaccent}$if(snapshot_label)$$snapshot_label$$else$Empirical Snapshot$endif$}\hfill
$if(date)$
\ifhfwganon\else
{\small $date$}%
\fi
...
\fcolorbox{hfwgaccent}{white}{%
  ...
  \centering\scriptsize\bfseries\color{hfwgaccent}\strut
  \end{minipage}%
}
\end{minipage}\hfill
\begin{minipage}[t]{0.76\textwidth}
\vspace{0pt}
{\small\sffamily\bfseries\color{hfwgaccent}$if(series)$$series$$else$HFWG Blog$endif$}\hfill
$if(date)$
\ifhfwganon\else
{\small $date$}%
\fi
...
{\small\bfseries\MakeUppercase{$if(series)$$series$$else$Demographic Research Note$endif$}%
$if(number)$\enspace\textbar\enspace No.\,$number$$endif$%
\hfill
...
{\setstretch{0.86}\huge\bfseries $title$\par}
...
$if(author)$
$for(author)$
$if(author.name)$\textbf{$author.name$}$else$\textbf{$author$}$endif$$sep$; $endfor$\par
$endif$
\vspace{0.35em}
$if(institution)$$institution$\par$else$--\par$endif$
\vspace{0.35em}
$if(series)$$series$$else$Demographic Research Note$endif$$if(number)$, No.\,$number$$endif$\par
\end{minipage}
...
$if(snapshot)$
$if(snapshot_feature)$
\newlength{\hfwgsnapshotfeatureheight}
\setlength{\hfwgsnapshotfeatureheight}{2.95in}
\noindent\makebox[\textwidth][r]{%
  \begin{minipage}[t]{0.50\textwidth}
  \vspace{0pt}
  \centering
  \includegraphics[width=\linewidth,height=2.45in,keepaspectratio]{$snapshot_feature$}
...
\vspace{-\hfwgsnapshotfeatureheight}
$endif$
\begin{multicols}{2}
...
\setlength{\parindent}{0.85em}
\setlength{\parskip}{0pt}
\titleformat{\section}{\small\bfseries\color{hfwgaccent}}{}{0pt}{}
\titleformat{\subsection}{\footnotesize\bfseries}{}{0pt}{}
\titlespacing{\section}{0pt}{0.45em}{0.12em}
\titlespacing{\subsection}{0pt}{0.35em}{0.1em}
\newcounter{hfwgsnapshotsection}
\let\hfwgsnapshotoldsection\section
\renewcommand{\section}[1]{%
  \stepcounter{hfwgsnapshotsection}%
  \ifnum\value{hfwgsnapshotsection}=3\relax
    \columnbreak
$if(snapshot_feature)$
    \vspace*{\hfwgsnapshotfeatureheight}%
$else$
...
$endif$
  \fi
  \hfwgsnapshotoldsection{#1}%
}
$if(abstract)$
\noindent{\bfseries $abstract$}\par\vspace{0.25em}
$endif$
$else$
\ifhfwghfwg
...
$if(snapshot_byline_link)${\color{hfwgaccent}$snapshot_byline_link$}\par$endif$
...
\scriptsize\bfseries\color{hfwgaccent}HFWG Snapshot
\end{minipage}\hfill
\begin{minipage}[t]{0.74\textwidth}
\scriptsize\raggedleft\itshape
\textbf{Data:} 2022 \emph{Survey of Consumer Finances.} For more information, visit: \href{https://hhfinance.commons.gc.cuny.edu}{hhfinance.commons.gc.cuny.edu}.
\end{minipage}\par
\endgroup
\restoregeometry
$endif$
...
$if(institution)$$institution$\par$endif$
$if(author_web)${\color{hfwgaccent}$author_web$}\par$endif$
