...
% =======================================================
\usepackage{graphicx}
%% eso-pic is only needed for the methods and hfwg title-page bars.
%% Loading it conditionally keeps default/humanities/demography free of the dependency.
\ifhfwgmethods\usepackage{eso-pic}\else\ifhfwghfwg\usepackage{eso-pic}\fi\fi
\usepackage{float}
\usepackage{multicol}
\usepackage{wrapfig}
...
{\setstretch{0.88}\Large\bfseries $title$\par}

$if(subtitle)$
\vspace{0.18em}
{\normalsize\bfseries\color{hfwgaccent} $subtitle$\par}
$endif$
...
%% Keep the title in the upper region; center the remaining metadata below it.
\vspace*{1in}%
\begin{center}
\begin{singlespace}
...
%% Blogposts keep the title close to the top rule; working papers center title-page content.
$if(blogpost)$
\vspace*{1in}%
$else$
\vspace*{3pt}%
...
\vspace{0.5em}
\noindent{\small\bfseries $if(series)$$series$$else$Demographic Research Note$endif$$if(number)$ \enspace No.\,$number$$endif$}\hfill{\small $if(date)$$date$$endif$}\par
\vspace{0.5em}
...
\vspace{-0.9em}
\includegraphics[width=0.43\linewidth,keepaspectratio]{$snapshot_feature$}
...
$if(snapshot)$
\par\vspace{0.45em}
\noindent\fcolorbox{hfwgrule}{white}{%
  ...
  $else$
  \fcolorbox{hfwgrule}{white}{\rule{0pt}{0.55in}\rule{0.48in}{0pt}}
  $endif$
  \end{minipage}\hfill
  ...
  $if(snapshot_byline_link)$$snapshot_byline_link$\par$endif$
