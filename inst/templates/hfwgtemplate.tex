...
$if(snapshot_label)$$snapshot_label$$else$Empirical Snapshot$endif$%
}\hfill
$if(date)$
\ifhfwganon\else
{\small $date$}%
...
\ifdim\wd0>\textwidth
  \noindent\resizebox{\textwidth}{!}{\Large\bfseries $title$}\par
\else
...
\vspace{0pt}
\includegraphics[width=\linewidth,keepaspectratio]{$snapshot_feature$}
...
\else\ifhfwghfwg
%% -------------------------------------------------------
%% HFWG: branded working-paper title page
%% Top and bottom bars at 1in from page edge.
%% All content vertically centered between the two bars.
%% -------------------------------------------------------
  ...
  \AtPageUpperLeft{%
    \put(\LenToUnit{\dimexpr 1in+\oddsidemargin\relax},\LenToUnit{-1in}){%
      \raisebox{-3pt}[0pt][0pt]{%
        \textcolor{hfwgaccent}{\rule{\textwidth}{3pt}}%
      }%
    }%
  }%
  \AtPageLowerLeft{%
...
%% Fixed upper title placement for both working papers and blogposts.
\vspace*{0.65in}%
\begin{center}
\begin{singlespace}
